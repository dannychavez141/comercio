...
$cadena=$_GET['cod'];
list($aludni, $apodni) = explode('_', $cadena);
$dboalumno= new dboAlumno();
$dboanio= new dboanioescolar();
$ultimoanio=$dboanio->ultimoanio();
$id=$dboalumno->ultimaMatriculadeAlumno($aludni,$ultimoanio[0]);
   $hoy = date('Y-m-d');
  $conexion=new cConexion();
$mysqli=$conexion->getBd();
    $sql = "SELECT dni FROM apoderado where dni='$apodni';";
...
//echo $sql;
    ...
    if ($deudas != 0) {
    header("Location: ./pdfnotificaciondeuda.php");
    }
...
while ($comp = $lista->fetch_array()) {
    ...
    if ($curso == $temp) {
        ...
        $pdf->Cell(11, $t, utf8_decode(notaprim($nota1, $modo)), 1, 0, 'C');
        if ($nota2 <= 0 && $modo == 1) {
            $pdf->SetTextColor(255, 0, 0);
        ...
        $pdf->Cell(10, $t, utf8_decode(notaprim($nota2, $modo)), 1, 0, 'C');
        if ($nota3 <= 0 && $modo == 1) {
            $pdf->SetTextColor(255, 0, 0);
        ...
        $pdf->Cell(10, $t, utf8_decode(notaprim($nota3, $modo)), 1, 0, 'C');
        if ($nota4 <= 0 && $modo == 1) {
            $pdf->SetTextColor(255, 0, 0);
        ...
        $pdf->Cell(11, $t, utf8_decode(notaprim($nota4, $modo)), 1, 1, 'C');
        ...
        if ($temp != "") {
            ...
            $pdf->Cell(11, 5, notaprim($prom1, $modo), 1, 0, 'C',1);
            if ($prom2 <= 0 && $modo == 1) {
                $pdf->SetTextColor(255, 0, 0);
            ...
            $pdf->Cell(10, 5, notaprim($prom2, $modo), 1, 0, 'C',1);
            if ($prom3 <= 0 && $modo == 1) {
                $pdf->SetTextColor(255, 0, 0);
            ...
            $pdf->Cell(10, 5, notaprim($prom3, $modo), 1, 0, 'C',1);
            if ($prom4 <= 0 && $modo == 1) {
                $pdf->SetTextColor(255, 0, 0);
            ...
            $pdf->Cell(11, 5, notaprim($prom4, $modo), 1, 1, 'C',1);
            $pdf->SetTextColor(0, 0, 0);
            $cont = 0;
            $suma1 = 0;
            $suma2 = 0;
            ...
            $pdf->Cell(16, $pos, notaprim($promedio, $modo), 1, 0, 'C');
            if ($recu <= 0 && $modo == 1) {
                $pdf->SetTextColor(255, 0, 0);
            ...
            $pdf->Cell(15, $pos, notaprim($recu, $modo), 1, 1, 'C');
            $pdf->SetTextColor(0, 0, 0);
            $pos = 0;
            $recu = 0;
            $y1 = $pdf->GetY();
...
$pdf->MultiCell(35, $pos, utf8_decode($temp), 1, 'C');
$salto = $salto + 1;
...
$recu = 0;
$modod = "I";
$nombre_archivo = "Boleta_de_Notas_" . $aludni . "_" .$ultimoanio[1]. ".pdf";
