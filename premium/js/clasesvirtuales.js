...
app.idgrado=getQueryVariable('idgrado');
console.log(app.idgrado);
...
Curso: <strong> " + decode_utf8(datosapi[i]['descr']) + "</strong> Codigo: " + datosapi[i]['codigo'] + " Clave: " + datosapi[i]['pass'] + "</td><td> <a href='" + datosapi[i]['url'] + "' class='btn btn-primary' target='_blank'> ENLACE A ZOOM</a></td></tr>";
    }
    combo += "</table>";
...
function llenarDireccion(datosapi) {
    ...
    for (var i = 0; i < datosapi.length; i++) {
        combo += "<tr align='center'><td>" + decode_utf8(datosapi[i]['prof']) + "<h5>DIRECTOR</h5>Codigo: " + datosapi[i]['codigo'] + " <br>Clave: " + datosapi[i]['pass'] + "<br><a href='" + datosapi[i]['url'] + "' class='btn btn-primary' target='_blank'> ENLACE A ZOOM </a></td></tr>";
    }
    combo += "</table>";
